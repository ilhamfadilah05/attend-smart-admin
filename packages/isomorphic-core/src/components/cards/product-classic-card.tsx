...
export default function ProductClassicCard({ product, className }: ProductProps) {
    ...
    <div className={cn("pb-0.5", className)}>
      ...
      <div className="pt-3">
        ...
        <div className="mt-2 flex items-center font-semibold text-gray-900">
          ...
          {price && <del className="ps-1.5 text-[13px] font-normal text-gray-500">{toCurrency(Number(price))}</del>}
        </div>
        <div className="mt-3">
          {isInCart(product.id) ? (
            <QuantityControl item={product} />
          ) : (
            <Button onClick={() => addItemToCart(product, 1)} className="w-full" variant="outline">
              Order
            </Button>
          )}
        </div>
...
function QuantityControl({ item }: { item: CartItem }) {
  ...
  return (
    <div className="flex h-10 w-full items-center justify-between rounded-md border border-gray-300 px-1 duration-200 hover:border-gray-900">
      ...
      <span className="grid w-8 place-content-center font-medium">{getItemFromCart(item.id).quantity}</span>
      <button
        title="Decrement"
        className="flex items-center justify-center rounded p-2 duration-200 hover:bg-gray-100 hover:text-gray-900"
