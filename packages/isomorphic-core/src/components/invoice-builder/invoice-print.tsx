...
import { InvoiceType } from "../../../../../src/validators/invoice-builder.schema";
...
export const InvoicePrint = forwardRef<HTMLDivElement, InvoicePrintProps>(({ data, totalTax, subTotal }, ref) => {
  return (
          ...
          <table className="w-full">
            ...
            <tbody>
              <tr>
                <td>
                  {/* first section  */}
                  <div className={cn("flex items-center justify-end", data.logo && "justify-between")}>
                    {data.logo && (
                      <Image alt="logo" width={80} height={80} src={data.logo!} className="h-24 w-32 object-contain" />
                    )}
                    <div className="text-end">
                      {data.company.name && (
                        <Text className="text-lg font-medium text-gray-900">{data.company.name}</Text>
                      )}
                  ...
                  <div className="mt-12 grid grid-cols-2 gap-8">
                    ...
                    {/* right side */}
                    <div className="text-end">
                      <Text className="mb-1 font-medium text-gray-900">{data.invoice.InvoiceNumberLabel}</Text>
                      <Text className="mb-5">{data.invoice.number}</Text>
                      <Text className="mb-1 font-medium text-gray-900">{data.invoice.InvoiceDateLabel}</Text>
                  ...
                  <div className="mt-12 w-full break-inside-auto border-b">
                    <div className="invoice-print-table-header !mb-2 grid break-inside-avoid grid-cols-12 gap-2 px-2 font-medium">
                      <span className="col-span-4 block py-2 text-start">{data.invoiceTableHeader.title}</span>
                      <span className="col-span-2 block py-2 text-end">{data.invoiceTableHeader.quantity}</span>
                      <span className="col-span-2 block py-2 text-end">{data.invoiceTableHeader.rate}</span>
                      <span className="col-span-2 block py-2 text-end">{data.invoiceTableHeader.tax}</span>
                      <span className="col-span-2 block py-2 text-end">{data.invoiceTableHeader.amount}</span>
                    </div>
                    {data.invoiceTable.map((item, index) => (
                      <div key={index} className="!mb-2 grid break-inside-avoid grid-cols-12 gap-2 px-2">
                        ...
                        <span className="col-span-2 block py-2 text-end">{item.quantity * item.rate}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="ms-auto mt-4 w-full max-w-[240px] break-inside-avoid divide-y font-medium">
                    ...
                    <div className="flex items-center justify-between gap-6 pt-2">
                      <span className="block text-gray-900">{data.finalCalculations.totalLabel}</span>
                  ...
                  <div className="break-before-auto break-inside-avoid-page space-y-2">
                    <Text className="text-lg font-semibold text-gray-900">{data.payments.label}</Text>
                  ...
                  {data.note.note && (
                    <div className="mt-12 break-before-auto break-inside-avoid-page">
                      <Text className="mb-1 font-medium text-gray-900">{data.note.noteLabel}</Text>
                  ...
                  {data.terms.terms && (
                    <div className="mt-12 break-before-auto break-inside-avoid-page">
                      <Text className="mb-1 font-medium text-gray-900">{data.terms.termsLabel}</Text>
                  ...
                  <div className="!mt-20 ms-auto grid w-full max-w-64 break-inside-avoid grid-cols-1 gap-1 text-center">
                    <Text className="text-lg font-semibold text-gray-900">{data.signature.name}</Text>
