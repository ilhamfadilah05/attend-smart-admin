...
import { ResponsiveContainer, XAxis, YAxis, Bar, LabelList, ComposedChart } from "recharts";
...
export default function Department({ className }: { className?: string }) {
  ...
  return (
    <WidgetCard
      ...
      action={<DropdownAction className="rounded-lg border" options={viewOptions} onChange={handleChange} />}
    >
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Bar dataKey="total" barSize={28} radius={[50, 50, 50, 50]}>
                <LabelList position="right" dataKey="total" content={<CustomizedLabel />} />
              </Bar>
            </ComposedChart>
          </ResponsiveContainer>
        </div>
...
function CustomizedLabel(props: any) {
  ...
  return (
    <g>
      <rect x={x + width - 44} y={y + 4} width={40} height={height - 8} rx={radius} fill="#ffffff" />
      <text
        y={y + 1 + height / 2}
        x={x + width - 24}
        fill="currentColor"
