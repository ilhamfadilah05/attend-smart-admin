...
export function useFilterControls<StateType, ActionType>(initialState: State<StateType>) {
  ...
  const paginate = useCallback(
    (pageNumber: string | number) => router.push(`${pathname}?${createQueryString("page", pageNumber.toString())}`),
  ...
  const params = Object.entries(parsedSearchParams).reduce((acc, [k, v]) => ({ ...acc, ...getQueryParams(k, v) }), {});
