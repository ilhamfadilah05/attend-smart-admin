...
const acceptedFileType = {
  ...
  all: "image/*,application/pdf,text/csv,application/gzip,application/xml,application/zip,application/msword,text/plain, .xlsx, .xls, .doc, .docx, .ppt, .pptx, .txt, .pdf",
  doc: ".xlsx, .xls, .doc, .docx, .ppt, .pptx, .txt, .pdf, text/csv, text/plain",
};
...
export interface UploadProps extends Omit<React.InputHTMLAttributes<HTMLInputElement>, "accept" | "children"> {
  ...
  accept: "img" | "pdf" | "csv" | "imgAndPdf" | "all" | "doc";
  /** Pass multiple files */
  multiple?: boolean;
...
function Upload(
  ...
  return (
    <div className={cn(wrapperClassName)}>
      {label && <label className={cn("mb-2 block text-sm font-medium", labelClassName)}>{label}</label>}
      <div
        className={cn(
          inputClasses.base,
          inputClasses.flex,
      ...
      >
        ...
        <div className="flex flex-col items-center @2xl:flex-row">
          <UploadIcon
            className={cn(
              variant !== "outline" && !props.disabled ? inputClasses.lightTextColor[color] : "text-gray-300",
              iconClassName
            )}
          />
          {placeholderText || (
