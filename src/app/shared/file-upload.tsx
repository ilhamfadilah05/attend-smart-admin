...
type AcceptedFiles = "img" | "pdf" | "csv" | "imgAndPdf" | "all" | "doc";
...
export default function FileUpload({
  ...
  onUpload?({ files }: FileProp): Promise<void>;
}) {
...
export const FileInput = ({
  ...
  function handleFileDrop(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (autoProcess && onUpload) {
      if (!newFiles?.length) {
        alertNotification("Harap Masukkan File", "error");
        return;
      }

      onUpload({ files: newFiles }).then((_) => {
        closeModal();
      });
    }
  }
  ...
  async function handleFileUpload() {
    if (files.length) {
      // alertNotification("File successfully added", "success");
