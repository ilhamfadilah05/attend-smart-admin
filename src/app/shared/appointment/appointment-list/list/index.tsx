...
export const appointmentTypesOptions = Object.entries(appointmentTypes).map(([value, label]) => ({ label, value }));
...
export default function AppointmentListTable() {
  ...
  const onChecked = (event: React.ChangeEvent<HTMLInputElement>, id: string) => {
  ...
  const columns = useMemo(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [onHeaderCellClick, sortConfig.key, sortConfig.direction, onDeleteItem, onChecked]
  );
  ...
  return (
    ...
    >
      <ControlledTable
        ...
        filterElement={
          <div className={cn("flex", isMediumScreen ? "flex-col gap-6" : "flex-row items-center gap-3")}>
            {!isMediumScreen && (
              <Title as="h3" className="rizzui-title-h3 pe-4 text-base font-semibold sm:text-lg">
                All Appointment
              </Title>
            )}
            ...
            <StatusField
              ...
              displayValue={(selected: string) =>
                appointmentTypesOptions.find((option) => option.label === selected)?.label ?? ""
              }
        ...
        tableFooter={
          ...
          >
            <Button size="sm" className="dark:bg-gray-300 dark:text-gray-800">
              Download {selectedRowKeys.length} {selectedRowKeys.length > 1 ? "Appointments" : "Appointment"}
            </Button>
          </TableFooter>
        }
      />
