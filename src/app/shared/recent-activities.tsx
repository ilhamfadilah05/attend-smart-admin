    ...
    <div className="relative flex items-start gap-x-2.5 pb-8 before:absolute before:start-[17px] before:top-0 before:z-0 before:h-full before:w-[1px] before:bg-gray-300 last:pb-0 last:before:hidden">
      ...
      <div className="">
        ...
        {files.map((file: string) => (
          <div key={file} className="mt-2 flex items-center gap-2.5 rounded-lg border border-gray-300 px-2 py-1.5">
...
export function ActivityThreads({ title, threads }: { title: string; threads: object[] }) {
  return (
    <div className="relative mb-4 last:mb-0">
      <Text className="mb-4 text-sm font-semibold text-gray-900 2xl:text-base dark:text-gray-700">{title}</Text>
      <div>
...
export default function RecentActivities({ className }: { className?: string }) {
  return (
    <div className={className}>
      <Title as="h3" className="mb-3 text-lg font-semibold text-gray-900 xl:text-xl 2xl:mb-5">
        Recent Activities
      </Title>
      <WidgetCard title="" headerClassName="hidden">
        {activities.map((activity, index) => (
          <ActivityThreads key={`thread-${index}`} title={activity.title} threads={activity.threads} />
        ))}
      </WidgetCard>
    </div>
  );
