...
import { DEFAULT_PRESET_COLOR_NAME, DEFAULT_PRESET_COLORS, usePresets } from "@/config/color-presets";
...
const colorPresetsAtom = atom(typeof window !== "undefined" ? getLocalStoragePreset() : DEFAULT_PRESET_COLORS);
...
export function useApplyColorPreset<T extends Record<string, any>>(colorPresets: T) {
  ...
  useEffect(() => {
    ...
    let colorForground = COLOR_PRESETS[0].colors.foreground;
    ...
    if (colorPresets) {
      ...
      colorForground = colorPresets.foreground;
    }

    updateThemeColor(colorLighter, colorLight, colorDefault, colorDark, colorForground);
