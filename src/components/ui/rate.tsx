  ...
  (
    ...
    return (
      <div className={cn("aegon-rate", className)}>
        {label && <div className={cn("block", labelClasses.size[size], labelClassName)}>{label}</div>}
        <RcRate
          ref={ref}
          disabled={disabled}
          characterRender={characterRender}
          character={({ index }: RcStarProps) => (
            <div className={cn("[&>svg]:fill-current", rateClasses.size[size], characterClassName)}>
              {Array.isArray(character) ? (character[index as number] as React.ReactNode) : character}
            </div>
          )}
          className={cn(
            rateClasses.base,
