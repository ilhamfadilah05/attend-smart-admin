...
export default function AppointmentDiseases({ className }: { className?: string }) {
  ...
  return (
    <WidgetCard
      ...
      className={cn("flex h-full w-full flex-col justify-between @container", className)}
      action={
        <DatePicker
          selected={startDate}
...
function CustomLegend({ className }: { className?: string }) {
  return (
    <div className={cn("flex flex-wrap gap-5 pt-5 @sm:flex-col @sm:pt-0 lg:gap-7", className)}>
      {data.map((item) => (
        <div key={item.name} className="relative flex w-2/5 flex-col ps-6 text-gray-500">
          <span
            className="absolute start-0 top-1/2 h-3 w-3 -translate-y-1/2 rounded"
            style={{ backgroundColor: item?.fill }}
          />
          {item.name}
          <span className="font-inter text-xl font-semibold text-gray-900">{item.patients}+</span>
        </div>
      ))}
    </div>
  );
