...
export function CartProvider({ cartKey, children, ...props }: { cartKey?: string; children: React.ReactNode }) {
  const [savedCart, saveCart] = useLocalStorage(cartKey ?? CART_KEY, JSON.stringify(initialState));
  ...
  const addItemsToCart = (items: Item[]) => dispatch({ type: "ADD_ITEMS_WITH_QUANTITY", items });
  ...
  const updateCartLanguage = (language: string) => dispatch({ type: "UPDATE_CART_LANGUAGE", language });
