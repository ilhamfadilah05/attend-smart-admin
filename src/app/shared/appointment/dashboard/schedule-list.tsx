  ...
  return (
    ...
    >
      <div className="grow p-5 @lg:w-3/5 lg:p-7">
        <Title as="h3" className="pb-4 font-inter text-base font-semibold text-gray-800 sm:text-lg">
          Schedule list
        </Title>
        <Calendar
          onChange={onChange}
      ...
      <div className="flex flex-col justify-between bg-gray-50 px-5 py-7 @lg:w-2/5 dark:bg-gray-100">
        <div className="mb-10 space-y-6 @lg:pt-2">
          {data.map((item) => (
            <Slider key={item.title} title={item.title} total={item.total} value={item.value} color={item.color} />
          ))}
        </div>
        <Button variant="outline" rounded="lg" className="fill-gray-900 text-gray-900">
...
function Slider({ title, total, value, color }: { title: string; total: number; value: number; color: string }) {
  ...
  return (
    <div className="group">
      ...
      <div className="relative h-2.5 w-full rounded-lg bg-gray-100 dark:bg-gray-200">
        <div style={{ width: `${percentage}%`, backgroundColor: color }} className="h-full rounded-lg " />
